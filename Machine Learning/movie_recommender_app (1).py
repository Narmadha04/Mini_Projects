...
def recommend_random_books(genre):
    # Filter books by the predicted genre
    genre_books = books[books['genre'] == genre]

    # Randomly select a fixed number of books (e.g., 5) from the entire book dataset
    num_books_to_recommend = 5

    if not genre_books.empty:
        # If there are books in the predicted genre, randomly select from them
        recommended_books = random.sample(list(genre_books['title']), num_books_to_recommend)
    else:
        # If there are no books in the predicted genre, randomly select from the entire book dataset
        all_books = list(books['title'])
        recommended_books = random.sample(all_books, num_books_to_recommend)
